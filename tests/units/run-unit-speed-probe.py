#!/usr/bin/env python3
"""Measure unit speeds and movement using real game objects.
...
include = root / 'tests/units/unit-speed-probe.inc'
...
if not args.record_baseline:
    with (root / 'tests/units/dynasty-unit-speeds.csv').open() as f:
        expected = {(r['unit'], r['terrain'], r['damaged'], r['loaded']): r for r in csv.DictReader(f)}
    for mod in ('vanilla', 'dunecity', 'Dune2R'):
        with (out / (mod + '.csv')).open() as f:
            for row in csv.DictReader(f):
                if row['unit'].startswith('route_'):
                    continue
                key = (row['unit'], row['terrain'], str(int(int(row['damaged']) > 0)), row['loaded'])
                ref = expected[key]
                for field in ('tiles_per_second', 'turn_degrees_per_second'):
                    if abs(float(row[field])-float(ref[field])) > 0.0001:
                        raise RuntimeError(f"{mod}: {row} disagrees with Dynasty: {ref}")
